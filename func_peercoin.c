...
void get_ip_txn(FILE *Pblock)
{
	...
	printf("Hash of previous transaction:");
	for (unsigned int i = 0; i<32; ++i)
	...
	printf("\nn=%u ", n); //n-Index refers to an output in the previous transaction

	scriptSigLength = varint(Pblock,ftell(Pblock)); //length of script ib bytes
	printf("\nscriptSigLength=%llu ", scriptSigLength);
	...
	printf("\ntransaction sequence =%u ", txn_sequence);
...
void get_op_txn(FILE *Pblock)
{
	...
	printf("\nscriptSigLength=%llu ", scriptPubKeyLength);
